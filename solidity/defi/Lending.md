...
AAVEV3:
1. Supply (Депозит): Пользователь вносит активы (ETH, DAI, USDC и т. д.) в пул ликвидности. -> Взамен получает процентный доход и (в некоторых протоколах) токены-признания депозита (например, aDAI в Aave или cDAI в Compound).
2. Borrow (Займ): Пользователь сначала должен внести залог (collateral).
3. Repay (Погашение займа)
4. Withdraw (Вывод депозита)
