...
> - Можно отправлять деньги, но будут заблокированы.
...
> indexed - если в событии указано поле indexed, то информация об этом поле будет сохранена в topics, что упрощает поиск. Причем максимальное количество индексируемых полей в topics - 3.
16. Что такое nonce?
> - nonce — это уникальный номер, связанный с каждой транзакцией, отправленной с определённого адреса. Nonce начинается с нуля и увеличивается на единицу с каждой последующей транзакцией
> - nonce гарантирует, что транзакции с одного адреса обрабатываются в строгом порядке их отправки, предотвращая ситуации, когда более поздняя транзакция обрабатывается раньше предыдущей
17. Можно ли сделать так, чтоб пользователь не платил за вызов функций?
> За read не платится. Иначе никак
18. Что такое библиотека?
>   Блок кода, который можно использовать
19. В каких случаях происходит встраивание, а в каких деплой библиотеки в контракт?
>   internal - встраивание, public - deploy
20. Что такое reentrancy?
>   Тип атаки, когда в рамках одной транзакции пытаемся обратиться к той же функции с целью списания большего количества средств.
21. Как защититься от reentrancy?
>   - Списывать баланс до call
>   - Модификатор noReentrancy
22. Что такое абстрактный класс?
> Класс, в котором по крайней мере одна функция не реализована, нельзя деплоить
> Или если класс, который наследует контракт, не вызывает конструктор родительского класса
23. Что такое delegatecall?
> Низкоуровневый вызов, который позволяет выполнять функцию другого контракта в своем контексте
24. Что такое staticcall?
> Низкоуровневый вызов, похожий на call, который может только выполнять read функции другого контракта, средства отправить нельзя. (более безопасен, чем call)
25. Как сгенерировать случайное число в solidity?
> Особо никак, но есть оракулы
26. Что такое fallback, receive?
> fallback вызовется, если в контракте придет транзакция с неизвестные селектором
> receive вызовется, если в транзакции приходят деньги, но не вызывается другая payable функция
27. Что такое интерфейс?
> Описание того, какие методы должен реализовывать контракт.
28. Что такое reference type?
> Переменная будет содержать не значение, а указатель на слот, где лежит переменная: string, mapping, [], struct
29. Какие объекты доступны глобально?
> - block.hash()
> - объект block
> - gasLeft() - сколько газа осталось
> - объект tx
> - объект message
> - объект this - указывает на текущий контракт
30. Что такое Yul?
...
34. Где хранятся события?

События в Ethereum записываются в логи транзакций. Логи не занимают места в основном состоянии блокчейна, а вместо этого они хранятся в специально отведенной области блокчейна, которая называется “логами событий” (**Event Logs**).

35.Адрес 0x40 в memory:
> По адресу 0x40 находится free memory pointer: хранит адрес начала свободной памяти
