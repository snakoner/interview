...
Структурно вектор хранится так:
```rust
struct Vec<T> {
    ptr: *mut T,    // Указатель на первый элемент (heap)
    len: usize,     // Текущее количество элементов (stack)
    cap: usize,     // Ёмкость (capacity) (stack)
}
```

При росте вектора: создается новый участок памяти (обычно *2). Все элементы копируются или перемещаются, если `T: !Copy`
...
> - Массивам фиксированной длины [i32; 3] (исключения: T не реализует trait Copy, массивы с .len() > 32 не реализуют Copy в стандартной библиотеке)
